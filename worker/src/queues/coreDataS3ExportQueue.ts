...
const getS3StorageServiceClient = (bucketName: string): StorageService => {
  if (!s3StorageServiceClient) {
    s3StorageServiceClient = StorageServiceFactory.getInstance({
      bucketName,
      accessKeyId: env.S3_CORE_DATA_UPLOAD_ACCESS_KEY_ID,
      secretAccessKey: env.S3_CORE_DATA_UPLOAD_SECRET_ACCESS_KEY,
      endpoint: env.S3_CORE_DATA_UPLOAD_ENDPOINT,
      region: env.S3_CORE_DATA_UPLOAD_REGION,
      forcePathStyle: env.S3_CORE_DATA_UPLOAD_FORCE_PATH_STYLE === "true",
      awsSse: env.S3_CORE_DATA_UPLOAD_SSE,
      awsSseKmsKeyId: env.S3_CORE_DATA_UPLOAD_SSE_KMS_KEY_ID,
    });
  }
...
export const coreDataS3ExportProcessor: Processor = async (): Promise<void> => {
  if (!env.S3_CORE_DATA_UPLOAD_BUCKET) {
    logger.error("No bucket name provided for core data S3 export");
    throw new Error("Must provide S3_CORE_DATA_UPLOAD_BUCKET to use core data S3 exports");
  }

  logger.info("Starting core data S3 export");

  const s3Client = getS3StorageServiceClient(env.S3_CORE_DATA_UPLOAD_BUCKET);
  ...
  await Promise.all(
    Object.entries({
      ...
      s3Client.uploadFile({
        fileName: `${env.S3_CORE_DATA_UPLOAD_PREFIX}${key}.jsonl`,
        fileType: "application/x-ndjson",
