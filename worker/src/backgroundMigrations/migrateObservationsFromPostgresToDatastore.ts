...
import { datastoreClient, convertPostgresObservationToInsert, logger } from "@hanzo/console/src/server";
...
export default class MigrateObservationsFromPostgresToDatastore implements IBackgroundMigration {
  ...
  private async updateMaxDate(stateSuffix: string, maxDate: Date) {
    await prisma.$transaction(
      async (tx) => {
        // @ts-ignore
        const migrationState: { state: Record<string, string> } = await tx.backgroundMigration.findUniqueOrThrow({
  ...
  async run(args: Record<string, unknown>): Promise<void> {
    ...
    const maxDate = initialDate ?? new Date((args.maxDate as string) ?? new Date());
    ...
    while (!this.isAborted && !this.isFinished && processedRows < maxRowsToProcess) {
      ...
      const observations = await prisma.$queryRaw<Array<Record<string, any>>>(Prisma.sql`
