...
import { datastoreClient, commandDatastore, logger, queryDatastore } from "@hanzo/console/src/server";
import { prisma } from "@hanzo/console/src/db";
import { env } from "../env";
import { parseArgs } from "node:util";
import { BaseChunkTodo, ConcurrentQueryManager, generateQueryId } from "./backfillEventsHistoric";

import { getQueryError, pollQueryStatus, sleep } from "@hanzo/console/src/server";
...
interface ChunkTodo extends BaseChunkTodo {
  partId: string; // Unique part identifier from Datastore
}
...
export default class BackfillEventsHistoricFromParts implements IBackgroundMigration {
  ...
  private async loadPartsFromDatastore(): Promise<ChunkTodo[]> {
    ...
    logger.info(`[Backfill Events] Loaded ${parts.length} parts from datastore system table`);
