...
import { datastoreClient, convertPostgresTraceToInsert, logger } from "@hanzo/console/src/server";
...
export default class MigrateTracesFromPostgresToDatastore implements IBackgroundMigration {
  ...
  async run(args: Record<string, unknown>): Promise<void> {
    ...
    logger.info(`Migrating traces from postgres to datastore with ${JSON.stringify(args)}`);
    ...
    while (!this.isAborted && !this.isFinished && processedRows < maxRowsToProcess) {
      ...
      logger.info(`Inserted ${traces.length} traces into Datastore in ${Date.now() - insertStart}ms`);
    ...
    if (this.isAborted) {
      logger.info(
        `Migration of traces from Postgres to Datastore aborted after processing ${processedRows} rows. Skipping cleanup.`,
      );
      return;
    }

    logger.info(`Finished migration of traces from Postgres to Datastore in ${Date.now() - start}ms`);
  }

  async abort(): Promise<void> {
    logger.info(`Aborting migration of traces from Postgres to datastore`);
    this.isAborted = true;
  }
}
...
async function main() {
  ...
  const migration = new MigrateTracesFromPostgresToDatastore();
