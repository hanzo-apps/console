...
import { type EvalTemplateCodeBased } from "@hanzo/console";
import {
...
export async function executeCodeBasedEvaluation(params: {
  ...
  return instrumentAsync({ name: "eval.execute-code-based-eval" }, async (span) => {
    ...
    span.setAttribute("eval.runner.language", params.template.sourceCodeLanguage);

    logger.debug(`Executing code-based evaluation for job ${jobExecutionId} in project ${params.projectId}`);
