...
} from "@hanzo/console/src/utils/chatml";
...
describe("ChatML Integration", () => {
  it("should handle OpenAI multimodal format", () => {
    ...
    const ctx = { metadata: { scope: { name: "hanzo-sdk" } } };
      ...
      candidates: [
        {
          content: {
            parts: [
              {
                text: "**Hanzo** is an **open-source observability and evaluation platform** for LLM applications.",
              },
            ],
            role: "model",
          },
  ...
  it("should handle Google Gemini format with function_call and function_response", () => {
    ...
    expect(inResult.data[3].content).toContain("Hello Hanzo");
  });
...
describe("ChatML adaption tests against real observations", () => {
  it.each(traceFiles)("should adapt observations from trace file %s ", (traceFile: string) => {
    //load trace file
    ...
    for (const obs of observations) {
      if (obs.input) {
        ...
        try {
          ...
          if (updateExpectedFilesOnFailure) writeToExpectedFile(expectedFile, obs.id, "input", normalizedInResult);
          else throw err;
        }
      }
      if (obs.output) {
        ...
        try {
          ...
          if (updateExpectedFilesOnFailure) writeToExpectedFile(expectedFile, obs.id, "output", normalizedOutResult);
          else throw err;
        }
      }
    }
 ...
 */
function writeToExpectedFile(expectedFileName: string, observationId: string, type: "input" | "output", data: any) {
  const fileDir = path.resolve(__dirname, "framework-traces");
