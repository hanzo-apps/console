...
describe("URL Normalization and Edge Cases", () => {
  describe("URL encoding bypass attempts", () => {
    it("should reject URL-encoded localhost", async () => {
      ...
      await expect(validateWebhookURL("http://%6C%6F%63%61%6C%68%6F%73%74/webhook")).rejects.toThrow(
    ...
    it("should reject double-encoded URLs", async () => {
      ...
      await expect(validateWebhookURL("http://%256C%256F%2563%2561%256C%2568%256F%2573%2574/webhook")).rejects.toThrow(
        /Invalid URL|Blocked hostname detected/,
      );
    });
    ...
    it("should handle partial URL encoding", async () => {
      ...
      await expect(validateWebhookURL("http://local%68ost/webhook")).rejects.toThrow("Blocked hostname detected");
    });

    it("should reject encoded private IPs", async () => {
      // %31%39%32%2E%31%36%38%2E%31%2E%31 = "192.168.1.1"
      await expect(validateWebhookURL("http://%31%39%32%2E%31%36%38%2E%31%2E%31/webhook")).rejects.toThrow(
  ...
  describe("Unicode and IDN bypass attempts", () => {
    it("should handle unicode domains properly", async () => {
      ...
      await expect(validateWebhookURL("http://тест.example.com/webhook")).rejects.toThrow(
  ...
  describe("Whitespace and special character handling", () => {
    it("should trim whitespace from URLs", async () => {
      await expect(validateWebhookURL("  http://localhost/webhook  ")).rejects.toThrow("Blocked hostname detected");
    });
    ...
    it("should handle URLs with unusual characters", async () => {
      await expect(validateWebhookURL("http://localhost:80/../webhook")).rejects.toThrow("Blocked hostname detected");
    });
  });
  ...
  describe("Protocol and port edge cases", () => {
    ...
    it("should handle implicit ports correctly", async () => {
      ...
      await expect(validateWebhookURL("https://localhost/webhook")).rejects.toThrow("Blocked hostname detected");
    });

    it("should reject non-standard port formats", async () => {
      await expect(validateWebhookURL("http://example.com:08080/webhook")).rejects.toThrow(
  ...
  describe("IPv6 edge cases", () => {
    it("should handle IPv6 with brackets", async () => {
      await expect(validateWebhookURL("http://[::1]/webhook")).rejects.toThrow(/Blocked IP address detected|ipaddr:/);
    });

    it("should handle IPv6 without brackets in URL context", async () => {
      // This should fail URL parsing
      await expect(validateWebhookURL("http://::1/webhook")).rejects.toThrow("Invalid URL syntax");
    });

    it("should handle compressed IPv6 addresses", async () => {
      await expect(validateWebhookURL("http://[2001:db8::1]/webhook")).rejects.toThrow(
        /Blocked IP address detected|ipaddr:/,
      );
    });

    it("should handle IPv4-mapped IPv6 addresses", async () => {
      await expect(validateWebhookURL("http://[::ffff:192.168.1.1]/webhook")).rejects.toThrow(
        /Blocked IP address detected|ipaddr:/,
      );
    });
  });
  ...
  describe("Malformed URL handling", () => {
    it("should reject URLs with invalid syntax", async () => {
      ...
      for (const url of invalidSyntaxUrls) {
        await expect(validateWebhookURL(url)).rejects.toThrow(/Invalid URL syntax/);
      }

      // Test URLs that fail at DNS resolution
      for (const url of invalidDnsUrls) {
        await expect(validateWebhookURL(url)).rejects.toThrow(/DNS lookup failed/);
      }
    });
    ...
    it("should handle URLs with invalid encoding", async () => {
      ...
      for (const url of invalidEncodedUrls) {
        await expect(validateWebhookURL(url)).rejects.toThrow(/Invalid URL|DNS lookup failed/);
      }
    });
  });
  ...
  describe("Boundary conditions", () => {
    it("should handle very long URLs", async () => {
      ...
      await expect(validateWebhookURL(`https://${longHostname}/webhook`)).rejects.toThrow(/DNS lookup failed/);
    });

    it("should handle URLs with many subdomains", async () => {
      const manySubdomains = Array(50).fill("sub").join(".") + ".example.com";
      await expect(validateWebhookURL(`https://${manySubdomains}/webhook`)).rejects.toThrow(/DNS lookup failed/);
    }, 10000);
    ...
    it("should handle empty hostname", async () => {
      ...
      await expect(validateWebhookURL("http:///webhook")).rejects.toThrow("DNS lookup failed for webhook");
    });
  });

  describe("Path and query parameter edge cases", () => {
    it("should validate hostname regardless of path", async () => {
      await expect(validateWebhookURL("http://localhost/very/long/path/with/many/segments")).rejects.toThrow(
        "Blocked hostname detected",
      );
    });

    it("should validate hostname regardless of query parameters", async () => {
      await expect(validateWebhookURL("http://localhost/webhook?param=value&other=test")).rejects.toThrow(
        "Blocked hostname detected",
      );
    });

    it("should validate hostname regardless of fragment", async () => {
      await expect(validateWebhookURL("http://localhost/webhook#fragment")).rejects.toThrow(
        "Blocked hostname detected",
      );
    });

    it("should handle encoded path components", async () => {
      await expect(validateWebhookURL("http://localhost/web%20hook")).rejects.toThrow("Blocked hostname detected");
    });
  });
  ...
  describe("Real-world bypass attempts", () => {
    it("should block attempts to access cloud metadata services", async () => {
      ...
      for (const url of metadataAttempts) {
        await expect(validateWebhookURL(url)).rejects.toThrow(/Blocked|DNS lookup failed/);
      }
    });
    ...
    it("should block attempts to access internal services", async () => {
      ...
      for (const url of internalAttempts) {
        await expect(validateWebhookURL(url)).rejects.toThrow(/Blocked|Only ports 80 and 443 are allowed/);
      }
    });
  });
});
