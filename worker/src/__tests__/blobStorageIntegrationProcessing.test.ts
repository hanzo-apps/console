import { expect, it, describe, beforeAll, beforeEach, afterEach, afterAll, vi } from "vitest";
...
import { BlobStorageIntegrationType, BlobStorageIntegrationFileType } from "@hanzo/console";
...
describe("BlobStorageIntegrationProcessingJob", () => {
  ...
  describe("gzip compression", () => {
    maybeIt("should produce .csv.gz files when compressed is true", async () => {
      const { projectId } = await createOrgProjectAndApiKey();
      s3Prefix = `${projectId}/`;
      const now = new Date();
      const oneHourAgo = new Date(now.getTime() - 60 * 60 * 1000);
      ...
      expect(projectFiles.every((f) => f.file.endsWith(".csv.gz"))).toBe(true);
    });

    maybeIt("should produce plain .csv files when compressed is false", async () => {
      const { projectId } = await createOrgProjectAndApiKey();
      s3Prefix = `${projectId}/`;
      const now = new Date();
      const oneHourAgo = new Date(now.getTime() - 60 * 60 * 1000);
      ...
      expect(projectFiles.length).toBeGreaterThan(0);
      expect(projectFiles.every((f) => f.file.endsWith(".csv") && !f.file.endsWith(".csv.gz"))).toBe(true);
    ...
    maybeIt("should produce .jsonl.gz files when compressed with JSONL format", async () => {
      const { projectId } = await createOrgProjectAndApiKey();
      s3Prefix = `${projectId}/`;
      const now = new Date();
      const oneHourAgo = new Date(now.getTime() - 60 * 60 * 1000);
      ...
      expect(projectFiles.length).toBeGreaterThan(0);
      expect(projectFiles.every((f) => f.file.endsWith(".jsonl.gz"))).toBe(true);
    });
  });
});
