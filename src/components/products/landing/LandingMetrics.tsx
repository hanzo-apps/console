...
/**
 * LandingMetrics — the live KPI row. Count-up values + real sparklines + honest deltas,
 * reusing the living-overview motion system (`useCountUp`/`usePoll`/`useReducedMotion`)
 * and the shared `Sparkline`. Two modes:
 ...
 * Honest throughout: a null value renders "—", a background poll failure keeps the last
 * real data, and a first-load failure shows a quiet note (metrics are supplementary —
 * never a full error card).
 */
...
export function LandingMetrics({ config }: { config: ProductLandingConfig }) {
  ...
  return (
    <XStack flexWrap="wrap" gap="$3">
      {metrics.map((m) => (
        <MetricTile key={m.key} metric={m} reduced={reduced} />
      ))}
    </XStack>
  )
}

function MetricTile({ metric, reduced }: { metric: LandingMetric; reduced: boolean }) {
