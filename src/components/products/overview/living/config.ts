 ...
 *
 * This is deliberately a SUPERSET of the shapes the console already produces
 * (`CloudUsageOverview` from `usage-adapter`, `AdminOverview` from
 * `admin-overview`, `FunctionsApi.metrics`): an adapter maps each real source onto
 * `OverviewData`, so wiring a product = one adapter + one config, never new UI.
 */
...
/**
 * A product's complete living overview — its header, its tiles (in render order),
 * its real data loader, and its live/motion settings. This IS the reusable unit:
 * `livingOverviewFor(id)` returns one of these; `LivingOverview` renders it.
 */
export type LivingOverviewConfig = {
  id: string
  title: string
  subtitle?: string
...
/**
 * A registered overview module component (what the registry route renders). `params`
 * is required to match `ProductRoute.component`; `allOrgs` is optional so the same
 * component doubles as the admin all-orgs god view (passed by admin.hanzo.ai's own
 * render, not the product router).
 */
export type LivingOverviewComponent = ComponentType<{ params: Record<string, string>; allOrgs?: boolean }>
