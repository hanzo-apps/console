/**
 * Living-overview motion — the PURE math behind the "videogame-like" feel, kept
 * out of the React components so it is unit-testable in the node vitest env (no
 * jsdom). The `.tsx` tiles are thin wrappers that drive `requestAnimationFrame`
 ...
 *   - `nextTick` — a self-correcting poll clock: given the last tick and now, it
