 ...
 * user-bearer proxy (`GET /v1/explorer/indexers`), which mints + forwards the caller's IAM
...
export function IndexerModule(_props: { params: Record<string, string> }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const r = await restGet<{ indexers?: Indexer[] }>(cloudProxyV1Url('explorer/indexers'))
      setRows(r.indexers ?? [])
      setLoadError(null)
    } catch (e) {
