 ...
 * `/v1/kb` surface (cloud `clients/knowledge`).
 *
 * `GET /v1/kb/graph` returns the org's knowledge as nodes (kb-page/kb-memory/
 * kb-source) + edges (parent tree, wikilinks, connector provenance), shaped for a
 * force-directed renderer. `POST /v1/kb/import?format=` ingests an Obsidian vault
 ...
 * same `/v1/kb/*` path reaches cloud directly under the session cookie.
 *
...
/** ImportResult is the `/v1/kb/import` response: how many pages were filed. */
...
export const KnowledgeApi = {
  ...
  graph: (project?: string): Promise<unknown> => {
    ...
    return restGet<unknown>(cloudProxyV1Url('kb/graph') + q)
  },
