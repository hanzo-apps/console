...
// server, so the service-token BFF route handlers (/billing/v1, /commerce/v1, /keys)
// are stripped by the static export and a request to them falls through to the SPA
// shell (HTML). The cloud binary serves the SAME heads at the CANONICAL bare /v1/* on
...
describe('embed-mode API paths (IS_EMBED)', () => {
  ...
  it('billing proxy → bare /v1/billing/<path>, never the stashed /billing/v1 handler', async () => {
    ...
    expect(billingProxyV1Url('balance')).not.toContain('/billing/v1/')
  })
  ...
  it('commerce proxy → bare /v1/commerce/<path>', async () => {
    ...
    expect(commerceProxyV1Url('product')).not.toContain('/commerce/v1/')
  })
