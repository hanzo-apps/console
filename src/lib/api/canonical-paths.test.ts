...
// no `/api/`), and (HIP-0139) each capability answers at ONE address — visor/*, platform/*,
// provisioning/*, account/*, ai/*. These surfaces all authorize on the Bearer owner claim
// and reject a cookie-only browser call ("X-Org-Id required" / "valid principal required" /
// a FALSE "session expired"). So the browser calls the canonical, prefix-free
// `<origin>/v1/<address>`, which terminates at the console's OWN `app/v1/[...path]` bearer
// BFF: it mints a short-lived user token from the session and forwards to cloud-api `/v1/*`
// with the org resolved from the token owner. This block PINS both the prefix-free contract
// and the folded address, so a regression can re-introduce neither a `/cloud/` prefix nor a
// second top-level address for a capability that already has one.
...
describe('canonical /v1 — session-scoped data-product clients (no prefix before /v1/)', () => {
  it('aicatalog fetchPlans -> /v1/plan/subscriptions (the subscription ladder)', async () => {
    stub({ plans: [] })
    await fetchPlans()
    expect(lastUrl).toBe(`${ORIGIN}/v1/plan/subscriptions`)
  })
...
describe('money + store + visor-catalog proxies (all /v1-first): /v1/billing, /v1/commerce, /v1/vm', () => {
  ...
  it('VisorApi.gpus (accelerator CATALOG) -> /v1/vm/gpus (visor, NOT the cloud inventory)', async () => {
    stub({ data: [] })
    await VisorApi.gpus()
