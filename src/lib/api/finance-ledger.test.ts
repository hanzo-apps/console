...
/**
 * The console finance transport addresses the `/cloud` user-bearer proxy (never a bare
 ...
 */
describe('financeUrl — /cloud proxy address (SSR, no window)', () => {
  it('builds the /cloud/v1/finance/<head> proxy URL', () => {
    expect(financeUrl('balance')).toBe('/cloud/v1/finance/balance')
    expect(financeUrl('payment-methods')).toBe('/cloud/v1/finance/payment-methods')
  })
  it('appends a query, skipping undefined values', () => {
    expect(financeUrl('usage', { range: '7d' })).toBe('/cloud/v1/finance/usage?range=7d')
    expect(financeUrl('ledger', { range: undefined })).toBe('/cloud/v1/finance/ledger')
  })
})
