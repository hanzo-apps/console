 ...
 * (`cloudProxyV1Url('crm/...')`): the server mints a short-lived user-bound IAM
 * token and the cloud backend resolves the org from the token's `owner` claim, so
 * every read/write is org-scoped SERVER-SIDE and no credential reaches the browser.
 * This is the SAME per-tenant path Prompts/Agents/Functions use (allow-listed as
 * the `crm` head in `proxy-allow.ts`). A cookie-only call would 403 ("X-Org-Id
 * required"), so the proxy is mandatory — never a same-origin cloud-origin call.
 *
...
export const CrmApi = {
  summary: (): Promise<Summary> => restGet<unknown>(cloudProxyV1Url(`${BASE}/summary`)).then(normalizeSummary),

  companies: {
    list: (): Promise<Company[]> =>
      restGet<unknown>(cloudProxyV1Url(`${BASE}/companies`)).then(normalizeCompanies),
    get: (id: string): Promise<Company> =>
      restGet<unknown>(cloudProxyV1Url(`${BASE}/companies/${enc(id)}`)).then(normalizeCompany),
    create: (body: NewCompany): Promise<Company> =>
      restPost<unknown>(cloudProxyV1Url(`${BASE}/companies`), body).then(normalizeCompany),
    update: (id: string, body: NewCompany): Promise<Company> =>
      restPut<unknown>(cloudProxyV1Url(`${BASE}/companies/${enc(id)}`), body).then(normalizeCompany),
    remove: (id: string): Promise<void> => restDelete(cloudProxyV1Url(`${BASE}/companies/${enc(id)}`)),
  },

  contacts: {
    list: (companyId?: string): Promise<Contact[]> =>
      restGet<unknown>(
        cloudProxyV1Url(`${BASE}/contacts${companyId ? `?companyId=${enc(companyId)}` : ''}`),
      ).then(normalizeContacts),
    get: (id: string): Promise<Contact> =>
      restGet<unknown>(cloudProxyV1Url(`${BASE}/contacts/${enc(id)}`)).then(normalizeContact),
    create: (body: NewContact): Promise<Contact> =>
      restPost<unknown>(cloudProxyV1Url(`${BASE}/contacts`), body).then(normalizeContact),
    update: (id: string, body: NewContact): Promise<Contact> =>
      restPut<unknown>(cloudProxyV1Url(`${BASE}/contacts/${enc(id)}`), body).then(normalizeContact),
    remove: (id: string): Promise<void> => restDelete(cloudProxyV1Url(`${BASE}/contacts/${enc(id)}`)),
  },

  opportunities: {
    list: (stage?: string): Promise<Opportunity[]> =>
      restGet<unknown>(
        cloudProxyV1Url(`${BASE}/opportunities${stage ? `?stage=${enc(stage)}` : ''}`),
      ).then(normalizeOpportunities),
    get: (id: string): Promise<Opportunity> =>
      restGet<unknown>(cloudProxyV1Url(`${BASE}/opportunities/${enc(id)}`)).then(normalizeOpportunity),
    create: (body: NewOpportunity): Promise<Opportunity> =>
      restPost<unknown>(cloudProxyV1Url(`${BASE}/opportunities`), body).then(normalizeOpportunity),
    update: (id: string, body: NewOpportunity): Promise<Opportunity> =>
      restPut<unknown>(cloudProxyV1Url(`${BASE}/opportunities/${enc(id)}`), body).then(normalizeOpportunity),
    remove: (id: string): Promise<void> =>
      restDelete(cloudProxyV1Url(`${BASE}/opportunities/${enc(id)}`)),
  },
}
