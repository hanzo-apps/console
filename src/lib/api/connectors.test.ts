 ...
 * `/v1/integrations/connectors` contract same-origin, keyless and prefix-free (`originV1Url` →
 * `<origin>/v1/integrations/connectors`); `next.config.mjs` rewrites that head to the user-bearer
 * `/v1` proxy. These tests pin (1) that each call hits the EXACT same-origin
 * `/v1/integrations/connectors` path with the right verb (the canonical Agents/CRM form — never a
...
describe('ConnectorsApi — hits the same-origin /v1/integrations/connectors contract (rewritten to the /v1 BFF)', () => {
  ...
  it('lists providers via GET the same-origin /v1/integrations/connectors path (not a direct cloud-origin call)', async () => {
    const out = await ConnectorsApi.list()
    expect(fetched[0]).toEqual({ url: `${ORIGIN}/v1/integrations/connectors`, method: 'GET' })
    expect(out.map((p) => p.id)).toEqual(['slack', 'github'])
  })

  it('gets one provider by id', async () => {
    const p = await ConnectorsApi.get('slack')
    expect(fetched[0]).toEqual({ url: `${ORIGIN}/v1/integrations/connectors/slack`, method: 'GET' })
