 ...
 * (clients/console/csrf.go): POST/DELETE `/v1/console/{keys,onboard,topup/wallet}`,
 ...
 *
 * The token is minted at `GET /v1/console/csrf` (its response body is unreadable to a
...
const csrfEndpoint = (): string =>
  typeof window !== 'undefined' ? `${window.location.origin}/v1/console/csrf` : '/v1/console/csrf'
...
/**
 * The current CSRF token, minted on demand from the same-origin `GET /v1/console/csrf`.
