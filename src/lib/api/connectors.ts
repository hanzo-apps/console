 ...
 * `/v1/integrations` surface (cloud `clients/connectors`, a generic provider-
 * agnostic OAuth connector store on Base/SQLite; Slack is the reference impl,
 * GitHub the registered surface, Google/Salesforce plug into the same registry).
 *
 ...
 *
 * Routes (from the CONNECTORS_CONTRACT, cloud `clients/connectors`). The connector
 * STORE is a leaf of the app — the provider rows it holds — while connect/disconnect
 * act on the provider itself and answer at the app's own root:
 *   GET  /v1/integrations/connectors           list → { providers: [Provider] }
 *   GET  /v1/integrations/connectors/:provider detail (404 unknown id)
...
export const ConnectorsApi = {
  ...
  list: (): Promise<Provider[]> =>
    restGet<unknown>(originV1Url(`${BASE}/connectors`)).then(normalizeProviders),

  /** One provider by id (404 → the caller's honest "unavailable"). */
  get: (id: string): Promise<Provider> =>
    restGet<unknown>(originV1Url(`${BASE}/connectors/${enc(id)}`)).then(normalizeProvider),
