...
/**
 * CRM API + pure normalizers. The module is wired to the DOCUMENTED cloud
 * `/v1/crm` contract through the console's OWN user-bearer `/cloud` proxy; these
 * tests pin (1) that each call hits that EXACT proxy path (never a cookie-only
 * cloud-origin call, which the crm surface 403s), (2) that the real store JSON
 * shape (store.go tags) normalizes, (3) list reads any envelope key, and (4) a
...
describe('CrmApi — hits the /v1/crm contract through the /cloud user-bearer proxy', () => {
  ...
  it('lists companies via the /cloud proxy (not a cookie-only cloud-origin call)', async () => {
    const out = await CrmApi.companies.list()
    expect(fetched[0]).toEqual({ url: `${ORIGIN}/cloud/v1/crm/companies`, method: 'GET' })
  ...
  it('creates a company with POST through the proxy', async () => {
    ...
    expect(fetched[0]).toEqual({ url: `${ORIGIN}/cloud/v1/crm/companies`, method: 'POST' })
  })

  it('reads the per-org summary', async () => {
    const s = await CrmApi.summary()
    expect(fetched[0].url).toBe(`${ORIGIN}/cloud/v1/crm/summary`)
  ...
  it('filters contacts by companyId and lists opportunities by stage', async () => {
    ...
    expect(fetched[0].url).toBe(`${ORIGIN}/cloud/v1/crm/contacts?companyId=comp_1`)
    expect(fetched[1].url).toBe(`${ORIGIN}/cloud/v1/crm/opportunities?stage=NEW`)
  })
})
