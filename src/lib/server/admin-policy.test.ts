...
describe('admin gate — IAM admin flag', () => {
  it('grants org or super admins, denies plain members', () => {
    expect(isAdminGranted({ isAdmin: true, isSuperAdmin: false })).toBe(true)
    expect(isAdminGranted({ isAdmin: false, isSuperAdmin: true })).toBe(true)
    expect(isAdminGranted({ isAdmin: false, isSuperAdmin: false })).toBe(false)
  })
})

describe('admin gate — combined (allow verified @hanzo.ai+admin, deny others)', () => {
  const admin = { email: 'z@hanzo.ai', emailVerified: true, isAdmin: true, isSuperAdmin: true }
  ...
  it('denies a non-brand email even when an IAM admin', () => {
    expect(gateAllows({ email: 'davelorenzini@gmail.com', emailVerified: true, isAdmin: true, isSuperAdmin: false }, 'hanzo.ai')).toBe(false)
  })
  it('denies a brand email that is NOT an IAM admin', () => {
    expect(gateAllows({ email: 'member@hanzo.ai', emailVerified: true, isAdmin: false, isSuperAdmin: false }, 'hanzo.ai')).toBe(false)
  })
  it('denies a brand admin whose email is NOT verified', () => {
    expect(gateAllows({ email: 'z@hanzo.ai', emailVerified: false, isAdmin: true, isSuperAdmin: true }, 'hanzo.ai')).toBe(false)
  })
})

describe('tenant scoping — ownerAllowed (cross-tenant read gap closed)', () => {
  const superAdmin = { isSuperAdmin: true, orgScope: 'hanzo' }
  const brand = { isSuperAdmin: false, orgScope: 'hanzo' }
  it('lets a SuperAdmin reference any org', () => {
    expect(ownerAllowed('adnexus', superAdmin)).toBe(true)
    expect(ownerAllowed('lux', superAdmin)).toBe(true)
  })
  ...
  it('allows the metadata owner only on org-list endpoints', () => {
    ...
    // matches commerce/ai/gateway. The canonical SuperAdmin metadata org is
...
describe('tenant scoping — orgFor (KMS never crosses a brand admin to another org)', () => {
  it('honors the requested org for a SuperAdmin', () => {
    expect(orgFor({ isSuperAdmin: true, orgScope: 'hanzo' }, 'adnexus')).toBe('adnexus')
  })
  it('falls back to the scope when a SuperAdmin requests nothing', () => {
    expect(orgFor({ isSuperAdmin: true, orgScope: 'hanzo' }, null)).toBe('hanzo')
  })
  it('IGNORES a requested org for a brand admin (no secret leak across orgs)', () => {
    expect(orgFor({ isSuperAdmin: false, orgScope: 'hanzo' }, 'adnexus')).toBe('hanzo')
  })
})

describe('org member proxy — orgWriteAllowed (customer can manage their OWN org)', () => {
  it('lets an org admin OR a SuperAdmin write; a plain member cannot', () => {
    // Dave/maxpower: org admin, not global → may invite/remove in his own org.
    expect(orgWriteAllowed({ isSuperAdmin: false, isAdmin: true })).toBe(true)
    expect(orgWriteAllowed({ isSuperAdmin: true, isAdmin: false })).toBe(true)
    // A non-admin member is read-only.
    expect(orgWriteAllowed({ isSuperAdmin: false, isAdmin: false })).toBe(false)
  })
})

describe('org member proxy — orgNameAllowed (no cross-org settings read)', () => {
  const brand = { isSuperAdmin: false, orgScope: 'maxpower' }
  it('pins a customer to their OWN org name, lets a SuperAdmin read any', () => {
    expect(orgNameAllowed('maxpower', brand)).toBe(true)
    expect(orgNameAllowed('hanzo', brand)).toBe(false) // another org's settings — denied
    expect(orgNameAllowed('hanzo', { isSuperAdmin: true, orgScope: 'maxpower' })).toBe(true)
