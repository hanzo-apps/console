...
export function overviewSection(brand: string): string {
  return [
    ...
    '- **Data** — managed primitives: **Base** (a Firebase-style backend — build content types and store records per org), **Vector** (embeddings + semantic search), **SQL**, **KV** (key-value cache/queues), **S3** object storage, **DocDB** (documents), **Datastore** (wide-column analytics), and **Memory**.',
