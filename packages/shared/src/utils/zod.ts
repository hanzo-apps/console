...
const nestedLiteralSchema = z.union([z.string(), z.number(), z.boolean(), z.null()]);
...
export const jsonSchemaNullable: z.ZodType<JsonNested> = z.lazy(() =>
  z.union([z.array(jsonSchemaNullable), z.record(z.string(), jsonSchemaNullable), nestedLiteralSchema]),
);

// Root schema that does not allow nulls at the root level
export const jsonSchema: z.ZodType<Json> = z.lazy(() =>
  z.union([z.array(jsonSchemaNullable), z.record(z.string(), jsonSchemaNullable), rootLiteralSchema]),
);

export const paginationZod = {
  page: z.preprocess((x) => (x === "" ? undefined : x), z.coerce.number().nonnegative().default(1)),
  limit: z.preprocess((x) => (x === "" ? undefined : x), z.coerce.number().nonnegative().lte(100).default(50)),
};

export const publicApiPaginationZod = {
  page: z.preprocess((x) => (x === "" ? undefined : x), z.coerce.number().gt(0).default(1)),
  limit: z.preprocess((x) => (x === "" ? undefined : x), z.coerce.number().lte(100).default(50)),
};

export const optionalPaginationZod = {
  page: z.preprocess((x) => (x === "" ? undefined : x), z.coerce.number()).optional(),
  limit: z.preprocess((x) => (x === "" ? undefined : x), z.coerce.number()).optional(),
};

export const queryStringZod = z.string().transform((val) => decodeURIComponent(val));
 ...
 */
export const validateZodSchema = <T extends z.ZodTypeAny>(schema: T, object: z.infer<T>): z.infer<T> => {
  return schema.parse(object);
};

// JSON Schema validation
export const JSONPrimitiveValueSchema = z.union([z.string(), z.number().finite(), z.boolean()]);

export const JSONValueSchema: z.ZodType<InputJsonValue> = z.lazy(() =>
  z.union([JSONPrimitiveValueSchema, z.array(JSONValueSchema), z.record(z.string(), JSONValueSchema)]),
);
