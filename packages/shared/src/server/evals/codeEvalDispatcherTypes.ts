...
export const CODE_EVAL_DOCS_URL = "https://langfuse.com/docs/evaluation/evaluation-methods/code-evaluators";

export function withCodeEvalDocs(message: string): string {
  const trimmedMessage = message.trim();
  const punctuatedMessage = /[.!?]$/.test(trimmedMessage) ? trimmedMessage : `${trimmedMessage}.`;
...
const CodeEvalScoreSchema = z.union([
  ...
  z.object({
    ...
    value: z.union([z.literal(0), z.literal(1), z.boolean(), z.string()]).transform((v, ctx): 0 | 1 => {
      if (v === 0 || v === 1) return v;
...
export type CodeEvalDispatcherErrorCode = z.infer<typeof CodeEvalDispatcherErrorCode>;
...
export function assertDispatchInputWithinLimits(input: DispatchInput): string {
  ...
  if (sourceBytes > CODE_EVAL_SOURCE_MAX_BYTES) {
    throw new CodeEvalDispatcherError(`Code eval source exceeds the ${CODE_EVAL_SOURCE_MAX_BYTES} byte limit`, {
 ...
 * downstream score ingestion and Datastore writes. Each dispatcher is
...
export function parseDispatchResult(result: unknown): DispatchResult {
  ...
  if (!parsed.success) {
    throw new CodeEvalDispatcherError(`Invalid code eval result: ${parsed.error.message}`, {
