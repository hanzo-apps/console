...
const CACHE_PREFIXES: Record<EvalConfigCacheType, string> = {
  traceBased: "hanzo:eval:no-trace-and-dataset-job-configs", // for target_object 'trace' | 'observation'
  eventBased: "hanzo:eval:no-event-and-experiment-job-configs", // for target_object 'event' | 'experiment'
};
 ...
 */
export const hasNoEvalConfigsCache = async (projectId: string, cacheType: EvalConfigCacheType): Promise<boolean> => {
  if (!redis) {
    return false;
  }
 ...
 */
export const setNoEvalConfigsCache = async (projectId: string, cacheType: EvalConfigCacheType): Promise<void> => {
  if (!redis) {
    return;
  }
  ...
  try {
    ...
    logger.debug(`Cached no ${cacheType} eval configs for project ${projectId}`);
  } catch (error) {
 ...
 */
export const clearNoEvalConfigsCache = async (projectId: string, cacheType: EvalConfigCacheType): Promise<void> => {
  if (!redis) {
    return;
  }
  ...
  try {
    ...
    logger.debug(`Cleared no ${cacheType} eval configs cache for project ${projectId}`);
  } catch (error) {
