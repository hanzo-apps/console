...
export class EvalExecutionQueue {
  private static instances: Map<number, Queue<TQueueJobTypes[QueueName.EvaluationExecution]> | null> = new Map();

  public static getShardNames() {
    return Array.from(
      { length: env.HANZO_EVAL_EXECUTION_QUEUE_SHARD_COUNT },
  ...
  static getShardIndexFromShardName(shardName: string | undefined): number | null {
    ...
    const shardIndex =
      shardName === QueueName.EvaluationExecution
        ? 0
        : parseInt(shardName.replace(`${QueueName.EvaluationExecution}-`, ""), 10);
  ...
  public static getInstance({
    ...
    const shardIndex =
      ...
      (env.REDIS_CLUSTER_ENABLED === "true" && shardingKey
        ? getShardIndex(shardingKey, env.HANZO_EVAL_EXECUTION_QUEUE_SHARD_COUNT)
        : 0);
...
export class SecondaryEvalExecutionQueue {
  private static instances: Map<number, Queue<TQueueJobTypes[QueueName.EvaluationExecutionSecondaryQueue]> | null> =
    new Map();

  public static getShardNames() {
    return Array.from(
      {
        length: env.HANZO_EVAL_EXECUTION_SECONDARY_QUEUE_SHARD_COUNT,
      },
      (_, i) => `${QueueName.EvaluationExecutionSecondaryQueue}${i > 0 ? `-${i}` : ""}`,
    );
  }
  ...
  static getShardIndexFromShardName(shardName: string | undefined): number | null {
    ...
    const shardIndex =
      shardName === QueueName.EvaluationExecutionSecondaryQueue
        ? 0
        : parseInt(shardName.replace(`${QueueName.EvaluationExecutionSecondaryQueue}-`, ""), 10);
  ...
  public static getInstance({
    ...
    const shardIndex =
      ...
      (env.REDIS_CLUSTER_ENABLED === "true" && shardingKey
        ? getShardIndex(shardingKey, env.HANZO_EVAL_EXECUTION_SECONDARY_QUEUE_SHARD_COUNT)
        : 0);
    ...
    const queueInstance = newRedis
      ? new Queue<TQueueJobTypes[QueueName.EvaluationExecutionSecondaryQueue]>(name, {
          connection: newRedis,
          prefix: getQueuePrefix(name),
          defaultJobOptions: {
            removeOnComplete: true,
