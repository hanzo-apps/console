...
export const checkTraceExistsAndGetTimestamp = async ({
  ...
  const observationFilter = tracesFilter.find((f) => f.datastoreTable === "observations");
  ...
  return measureAndReturn({
    ...
    input: {
      params: {
        ...
        ...(maxTimeStamp ? { maxTimeStamp: convertDateToDatastoreDateTime(maxTimeStamp) } : {}),
        ...(exactTimestamp ? { exactTimestamp: convertDateToDatastoreDateTime(exactTimestamp) } : {}),
      },
      tags: {
        feature: "tracing",
        type: "trace",
    ...
    fn: async (input) => {
      ...
      return {
        ...
        timestamp: rows.length > 0 ? parseDatastoreUTCDateTimeFormat(rows[0].timestamp) : undefined,
      };
    },
  });
};
...
export const upsertTrace = async (trace: Partial<TraceRecordReadType>) => {
  ...
  await upsertDatastore({
    ...
    eventBodyMapper: convertDatastoreToDomain,
    tags: {
      feature: "tracing",
      type: "trace",
      kind: "upsert",
  ...
  const records = await measureAndReturn({
    ...
    input: {
      params: {
        traceIds,
        projectId,
        timestamp: timestamp ? convertDateToDatastoreDateTime(timestamp) : null,
      },
      tags: {
        feature: "tracing",
        type: "trace",
    ...
    fn: (input) => {
      ...
      return queryDatastore<TraceRecordReadType>({
        ...
        params: input.params,
        tags: input.tags,
        datastoreConfig: input.datastoreConfig,
      });
    },
  });

  return records.map((record) => convertDatastoreToDomain(record, DEFAULT_RENDERING_PROPS));
};

export const getTracesBySessionId = async (projectId: string, sessionIds: string[], timestamp?: Date) => {
  const records = await measureAndReturn({
  ...
  const traces = records.map((record) => convertDatastoreToDomain(record, DEFAULT_RENDERING_PROPS));

  traces.forEach((trace) => {
    recordDistribution("hanzo.traces_by_session_id_age", new Date().getTime() - trace.timestamp.getTime());
  });
...
export const getTracesGroupedByName = async (
  ...
  const chFilter = timestampFilter ? createFilterFromFilterState(timestampFilter, tableDefinitions) : undefined;
...
export const getTracesGroupedBySessionId = async (
  ...
  tracesFilter.push(...createFilterFromFilterState(filter, columns ?? tracesTableUiColumnDefinitions));

  const tracesFilterRes = tracesFilter.apply();
  const search = datastoreSearchCondition(searchQuery, undefined, "t");
...
export const getTracesIdentifierForSession = async (projectId: string, sessionId: string) => {
...
export const hasAnyTraceOlderThan = async (projectId: string, beforeDate: Date) => {
  const query = `
    SELECT 1
    FROM traces
    WHERE project_id = {projectId: String}
...
export const deleteTracesOlderThanDays = async (projectId: string, beforeDate: Date): Promise<boolean> => {
...
export const deleteTracesByProjectId = async (projectId: string): Promise<boolean> => {
...
export const getUserMetrics = async (projectId: string, userIds: string[], filter: FilterState) => {
  if (userIds.length === 0) {
    return [];
  }

  // filter state contains date range filter for traces so far.
  const chFilter = new FilterList(createFilterFromFilterState(filter, tracesTableUiColumnDefinitions));
  const chFilterRes = chFilter.apply();

  const timestampFilter = chFilter.find((f) => f.field === "timestamp" && f.operator === ">=");
  ...
  return measureAndReturn({
    ...
    input: {
      params: {
        ...
        ...(timestampFilter
          ? {
              traceTimestamp: convertDateToDatastoreDateTime((timestampFilter as DateTimeFilter).value),
            }
          : {}),
      },
      tags: {
...
export const getTracesForBlobStorageExport = function (projectId: string, minTimestamp: Date, maxTimestamp: Date) {
 ...
 *
 */
export const getTraceCountsByProjectAndDay = async ({ startDate, endDate }: { startDate: Date; endDate: Date }) => {
  const query = `
    SELECT
      count(*) as count,
      project_id,
