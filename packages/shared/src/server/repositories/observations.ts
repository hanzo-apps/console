...
export const checkObservationExists = async (
  ...
  const rows = await queryDatastore<{ id: string; project_id: string }>({
    query,
    params: {
      id,
      projectId,
      ...(startTime ? { startTime: convertDateToDatastoreDateTime(startTime) } : {}),
    },
    tags: {
      feature: "tracing",
      type: "observation",
...
export const getObservationsForTrace = async <IncludeIO extends boolean>(
  ...
  const { traceId, projectId, timestamp, includeIO = false, preferredService } = opts;
  ...
  const records = await queryDatastore<ObservationRecordReadType>({
    query,
    params: {
      traceId,
      projectId,
...
export const getObservationsById = async (ids: string[], projectId: string, fetchWithInputOutput: boolean = false) => {
  const query = `
  SELECT
    id,
    trace_id,
...
export const getObservationsTableWithModelData = async (opts: ObservationTableQuery): Promise<FullObservations> => {
...
const getObservationsTableInternal = async <T>(
  ...
  const { projectId, filter, selectIOAndMetadata, limit, offset, orderBy, datastoreConfig } = opts;
  ...
  const timeFilter = filter.find((f) => f.column === "Start Time" && (f.operator === ">=" || f.operator === ">"));
  ...
  const traceTableFilter = filter.filter((f) =>
    observationsTableTraceUiColumnDefinitions.some((c) => c.uiTableId === f.column || c.uiTableName === f.column),
  );

  const orderByTraces = orderBy
    ? observationsTableTraceUiColumnDefinitions.some(
        (c) => c.uiTableId === orderBy.column || c.uiTableName === orderBy.column,
      )
    : undefined;
  ...
  observationsFilter.push(...createFilterFromFilterState(filter, observationsTableUiColumnDefinitions));

  const appliedScoresFilter = scoresFilter.apply();
  const appliedObservationsFilter = observationsFilter.apply();

  const search = datastoreSearchCondition(opts.searchQuery, opts.searchType, "o");
  ...
  const chOrderBy = orderByToDatastoreSql(newDefaultOrder, [
    ...observationsTableUiColumnDefinitions,
    {
      ...
      datastoreTableName: "observation",
      datastoreSelect: "toDate(o.start_time)",
    },
  ]);
  ...
  return measureAndReturn({
    ...
    input: {
      params: {
        ...
        ...(timeFilter
          ? {
              tracesTimestampFilter: convertDateToDatastoreDateTime(timeFilter.value as Date),
            }
          : {}),
        ...search.params,
      },
...
export const getObservationsGroupedByModel = async (projectId: string, filter: FilterState) => {
  ...
  const res = await queryDatastore<{ name: string }>({
    query,
    params: {
      ...appliedObservationsFilter.params,
    },
...
export const getObservationsGroupedByModelId = async (projectId: string, filter: FilterState) => {
  ...
  const res = await queryDatastore<{ modelId: string }>({
    query,
    params: {
      ...appliedObservationsFilter.params,
    },
...
export const getObservationsGroupedByToolName = async (projectId: string, filter: FilterState) => {
  ...
  const res = await queryDatastore<{ toolName: string }>({
    query,
    params: {
      ...appliedObservationsFilter.params,
    },
...
export const getObservationsGroupedByCalledToolName = async (projectId: string, filter: FilterState) => {
  const observationsFilter = new FilterList([
    new StringFilter({
      datastoreTable: "observations",
      field: "project_id",
  ...
  const res = await queryDatastore<{ calledToolName: string }>({
    query,
    params: {
      ...appliedObservationsFilter.params,
    },
...
export const getObservationsGroupedByPromptName = async (projectId: string, filter: FilterState) => {
  const observationsFilter = new FilterList([
    new StringFilter({
      datastoreTable: "observations",
      field: "project_id",
  ...
  const res = await queryDatastore<{ id: string }>({
    query,
    params: {
      ...appliedObservationsFilter.params,
    },
...
export const getCostForTraces = async (projectId: string, timestamp: Date, traceIds: string[]) => {
 ...
 `;
  ...
  const res = await queryDatastore<{ total_cost: string }>({
    query,
    params: {
      projectId,
      traceIds,
...
export const deleteObservationsOlderThanDays = async (projectId: string, beforeDate: Date): Promise<boolean> => {
...
`;
  const rows = await queryDatastore<{ count: string; prompt_name: string }>({
    query: query,
    params: {
      projectId,
      promptNames,
...
`;
  ...
  return rows.map((r) => ({
    ...
    firstObservation: parseDatastoreUTCDateTimeFormat(r.first_observation),
    lastObservation: parseDatastoreUTCDateTimeFormat(r.last_observation),
...
`;
  const rows = await queryDatastore<{
    ...
    params: {
      projectId,
      observationIds,
      ...(timestamp ? { timestamp: convertDateToDatastoreDateTime(timestamp) } : {}),
    },
    tags: {
      feature: "tracing",
      type: "observation",
...
export const getObservationsGroupedByTraceId = async (
  ...
  const groupedObservations = await queryDatastore<{
    trace_id: string;
...
export const getObservationCountsByProjectInCreationInterval = async ({ start, end }: { start: Date; end: Date }) => {
  const query = `
    SELECT
      project_id,
      count(*) as count
...
export const getTraceIdsForObservations = async (projectId: string, observationIds: string[]) => {
  ...
  const rows = await queryDatastore<{ id: string; trace_id: string }>({
    query,
    params: {
      projectId,
      observationIds,
...
export const getGenerationsForAnalyticsIntegrations = async function* (
  ...
  for await (const record of records) {
    yield {
      ...
      console_generation_name: record.name,
      ...
      console_session_id: record.trace_session_id,
