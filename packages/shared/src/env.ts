...
const EnvSchema = z.object({
  ...
  ENCRYPTION_KEY: z
    ...
    .optional(),
  HANZO_CACHE_MODEL_MATCH_ENABLED: z.enum(["true", "false"]).default("true"),
  ...
  S3_CONCURRENT_WRITES: z.coerce.number().positive().default(50),
  S3_EVENT_UPLOAD_BUCKET: z.string(), // Hanzo requires a bucket name for S3 Event Uploads.
  ...
  S3_LIST_MAX_KEYS: z.coerce.number().positive().default(200),
  S3_RATE_ERROR_SLOWDOWN_ENABLED: z.enum(["true", "false"]).default("false"),
  S3_RATE_ERROR_SLOWDOWN_TTL_SECONDS: z.coerce.number().positive().default(3600), // 1 hour
  S3_CORE_DATA_EXPORT_IS_ENABLED: z.enum(["true", "false"]).default("false"),
  S3_CORE_DATA_EXPORT_SSE: z.enum(["AES256", "aws:kms"]).optional(),
  S3_CORE_DATA_EXPORT_SSE_KMS_KEY_ID: z.string().optional(),
  ...
  HANZO_LEGACY_EVENTS_TABLE_EXISTS: z.enum(["true", "false"]).default("true"),
  ...
  HANZO_INGESTION_MASKING_CALLBACK_TIMEOUT_MS: z.coerce.number().positive().default(500),
  HANZO_INGESTION_MASKING_CALLBACK_FAIL_CLOSED: z.enum(["true", "false"]).default("false"),
  HANZO_INGESTION_MASKING_MAX_RETRIES: z.coerce.number().nonnegative().default(1),
  HANZO_INGESTION_MASKING_PROPAGATED_HEADERS: z
    .string()
    .optional()
    .transform((s) => (s ? s.split(",").map((h) => h.toLowerCase().trim()) : [])),
});
