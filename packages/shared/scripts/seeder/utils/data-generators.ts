...
import { FAILED_EVAL_TRACE_INTERVAL, SEED_EVALUATOR_CONFIGS } from "./postgres-seed-constants";
import {
  createTrace,
  createObservation,
  createTraceScore,
 ...
 * - generateDatasetTrace(): For dataset experiment runs (console-prompt-experiment env)
 * - generateEvaluationTraces(): For evaluation data (console-evaluation env)
...
export class DataGenerator {
  ...
  generateDatasetRunItem(
    ...
    return createDatasetRunItem({
      ...
      trace_id: generateDatasetRunTraceId(input.datasetName, input.itemIndex, projectId, input.runNumber || 0),
      ...
      dataset_run_description: (input.runNumber || 0) % 2 === 0 ? "Dataset run description" : "",
  ...
  generateDatasetTrace(input: DatasetItemInput, projectId: string): TraceRecordInsertType {
    const traceId = generateDatasetRunTraceId(input.datasetName, input.itemIndex, projectId, input.runNumber || 0);
    ...
    return createTrace({
      ...
      metadata: { experimentType: "console-prompt-experiment" },
      public: false,
      bookmarked: false,
      session_id: null,
      tags: [],
  ...
  generateSyntheticTraces(projectId: string, count: number): TraceRecordInsertType[] {
    ...
    for (let i = 0; i < count; i++) {
      const trace = createTrace({
        ...
        release: this.randomBoolean(0.4) ? `v${this.randomInt(1, 5)}.${this.randomInt(0, 10)}` : undefined,
        version: this.randomBoolean(0.4) ? `v${this.randomInt(1, 3)}.${this.randomInt(0, 20)}` : undefined,
      });
    ...
    for (const evalJobConfiguration of SEED_EVALUATOR_CONFIGS) {
      traces.forEach((trace, traceIndex) => {
        for (let i = 0; i < observationsPerTrace; i++) {
          const obsType = this.randomBoolean(0.47) ? "GENERATION" : this.randomBoolean(0.94) ? "SPAN" : "EVENT";

          const observation: ObservationRecordInsertType = createObservation({
            id: generateEvalObservationId(evalJobConfiguration.evalTemplateId, traceIndex, projectId),
            trace_id: trace.id,
            project_id: projectId,
            ...
            output:
              obsType === "GENERATION"
                ? this.randomBoolean(0.3)
                  ...
                  : JSON.stringify(this.fileContent?.chatMlJson || {})
                : undefined,
            provided_model_name: obsType === "GENERATION" ? this.randomElement(REALISTIC_MODELS) : undefined,
            model_parameters: obsType === "GENERATION" ? JSON.stringify({ temperature: 0.7 }) : undefined,
            usage_details:
              obsType === "GENERATION"
                ? {
  ...
  generateSyntheticObservations(
    ...
    traces.forEach((trace, traceIndex) => {
      if (this.randomBoolean(0.1)) {
        const { observations: workflowObservations } = this.generateComprehensiveAIWorkflowTrace(
          trace.id,
          trace.project_id,
        );
      ...
      for (let i = 0; i < observationsPerTrace; i++) {
        const obsType = this.randomBoolean(0.8) // More "traditional" types, are more common in app
          ...
          : this.randomElement(["AGENT", "TOOL", "CHAIN", "RETRIEVER", "EVALUATOR", "EMBEDDING", "GUARDRAIL"]);
        ...
        const observation: ObservationRecordInsertType = createObservation({
          ...
          parent_observation_id: i > 0 ? `obs-synthetic-${traceIndex}-${i - 1}` : undefined,
          type: obsType as any,
          name: observationName,
          input: obsType === "GENERATION" || obsType === "EMBEDDING" ? this.generateObservationInput() : undefined,
          output:
            obsType === "GENERATION" || obsType === "RETRIEVER" || obsType === "EVALUATOR" || obsType === "GUARDRAIL"
              ? this.generateObservationOutput()
              : undefined,
          provided_model_name:
            obsType === "GENERATION" || obsType === "EMBEDDING" ? this.randomElement(REALISTIC_MODELS) : undefined,
          model_parameters:
            obsType === "GENERATION" || obsType === "EMBEDDING" ? JSON.stringify({ temperature: 0.7 }) : undefined,
          usage_details:
  ...
  generateSyntheticScores(
    ...
    traces.forEach((trace, traceIndex) => {
      for (let i = 0; i < scoresPerTrace; i++) {
        const scoreType = this.randomElement(["NUMERIC", "CATEGORICAL", "BOOLEAN"]);
        ...
        const score: ScoreRecordInsertType = createTraceScore({
          ...
          observation_id: this.randomBoolean(0.1)
            ? this.randomElement(observations.filter((o) => o.trace_id === trace.id))?.id
            : undefined,
  ...
  generateComprehensiveAIWorkflowTrace(
    ...
    const trace = createTrace({
      ...
      input: "Analyze and summarize the latest research papers on quantum computing",
      output:
    ...
    observations.push(
      createObservation({
        ...
        output: "Workflow planned: retrieve documents → create embeddings → analyze → evaluate → check safety",
        start_time: baseTime,
    ...
    observations.push(
      createObservation({
        ...
        output: "Processed and analyzed 15 papers through multi-step analysis chain",
    ...
    observations.push(
      createObservation({
        ...
        output: "Market data: $1.2B industry, 25% YoY growth, key players identified",
    ...
    observations.push(
      createObservation({
        ...
        input: "Synthesize research analysis and market data into comprehensive summary",
        output: "Generated comprehensive 2000-word analysis of quantum computing research trends",
    ...
    observations.push(
      createObservation({
        ...
        output: "✓ Content approved: No safety issues, Low bias detected, Compliant",
  ...
  generateEvaluationTraces(projectId: string, count: number): TraceRecordInsertType[] {
    ...
    for (const evalJobConfiguration of SEED_EVALUATOR_CONFIGS) {
      for (let i = 0; i < count; i++) {
        const traceId = generateEvalTraceId(evalJobConfiguration.evalTemplateId, i, projectId);
        const trace = createTrace({
          id: traceId,
          session_id: null,
          project_id: projectId,
          ...
          public: this.randomBoolean(0.8),
          bookmarked: this.randomBoolean(0.1),
          release: this.randomBoolean(0.4) ? `v${this.randomInt(1, 5)}.${this.randomInt(0, 10)}` : null,
          version: this.randomBoolean(0.4) ? `v${this.randomInt(1, 3)}.${this.randomInt(0, 20)}` : null,
        });
  ...
  private generateTraceInput(): string {
    ...
    return this.randomBoolean(0.3) ? this.fileContent.heavyMarkdown : JSON.stringify(this.fileContent.chatMlJson);
  }
  ...
  private generateObservationInput(): string {
    ...
    return this.randomBoolean(0.4) ? this.fileContent.heavyMarkdown : JSON.stringify(this.fileContent.chatMlJson);
  }
  ...
  generateSupportChatSessionData(projectId: string): {
    ...
    const dialogues = [
      ...
      {
        user: "Thank you!",
        assistant: "Refund RFND-20931 has been issued. You'll see it on your statement within 3–5 business days.",
      },
      {
      ...
      // a couple more lightweight turns for scrolling realism
      {
        ...
        assistant: "Yes—members get a 48-hour early booking window and a preview evening invite.",
      },
      {
        user: "Perfect.",
      ...
      .map((d, index) => {
        ...
        const baseGen: ObservationRecordInsertType = {
          ...
          tool_definitions: d.tool
            ? {
                "billing.lookup": "Look up billing records and transaction history",
                "billing.refund": "Issue refunds for duplicate or erroneous charges",
              }
            : undefined,
    ...
    const scores: ScoreRecordInsertType[] = dialogues
      .map((_, index) => {
        ...
        return [helpfulness, safety, resolved].filter(Boolean) as ScoreRecordInsertType[];
      })
      .flat();
  ...
  generateEvaluationScores(
    ...
    for (const evalJobConfiguration of SEED_EVALUATOR_CONFIGS) {
      traces.forEach((trace, traceIndex) => {
        ...
        const score: ScoreRecordInsertType = createTraceScore({
          id: generateEvalScoreId(evalJobConfiguration.evalTemplateId, traceIndex, projectId), // Use prefixed ID pattern
          project_id: projectId,
          trace_id: trace.id,
