...
export const notificationPreferencesRouter = createTRPCRouter({
  getForProject: protectedProjectProcedure.input(GetPreferencesInput).query(async ({ input, ctx }) => {
  ...
  update: protectedProjectProcedure.input(NotificationPreferenceInput).mutation(async ({ input, ctx }) => {
    throwIfNoProjectAccess({
      session: ctx.session,
      projectId: input.projectId,
      scope: "project:read",
