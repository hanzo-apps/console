...
export async function getAuthOptions(): Promise<NextAuthOptions> {
  ...
  // Add Hanzo IAM provider (OIDC, PKCE S256, endpoints resolved via discovery)
  if (env.IAM_CLIENT_ID && env.IAM_CLIENT_SECRET && env.IAM_SERVER_URL)
    data.providers.push({
      ...
      // Discovery is the single source of truth for authorize/token/userinfo/jwks.
      // The document advertises the canonical /v1/iam/oauth/* endpoints — never hardcode them here.
      wellKnown: `${env.IAM_SERVER_URL}/.well-known/openid-configuration`,
      authorization: {
        params: {
          scope: "openid email profile",
          response_type: "code",
