...
import { makeIdempotencyKey, IdempotencyKind } from "@/src/ee/features/billing/utils/stripeIdempotencyKey";
...
type SubscriptionWithSchedule = ExpandedNullable<Stripe.Subscription, "schedule">;
...
class BillingService {
  ...
  private async retrieveSubscriptionWithScheduleAndDiscounts(
    ...
    return await instrumentAsync({ name: "stripe.subscription.retrieve", spanKind: SpanKind.CLIENT }, async (span) => {
  ...
  private async retrieveProductWithDefaultPrice(client: Stripe, productId: string): Promise<ProductWithDefaultPrice> {
    return await instrumentAsync({ name: "stripe.product.retrieve", spanKind: SpanKind.CLIENT }, async (span) => {
  ...
  private async retrieveInvoiceList(
    ...
    return await instrumentAsync({ name: "stripe.invoices.list", spanKind: SpanKind.CLIENT }, async (span) => {
      span.setAttributes({
  ...
  private async createInvoicePreview(client: Stripe, stripeCustomerId: string, subscriptionId: string) {
    return await instrumentAsync({ name: "stripe.invoice.preview", spanKind: SpanKind.CLIENT }, async (span) => {
      span.setAttributes({
        "stripe.customer_id": stripeCustomerId,
  ...
  private async releaseExistingSubscriptionScheduleIfAny(subscription: SubscriptionWithSchedule, opId?: string) {
  ...
  async getSubscriptionInfo(orgId: string): Promise<BillingSubscriptionInfo> {
    return await instrumentAsync({ name: "stripe.subscription.getInfo", spanKind: SpanKind.CLIENT }, async (span) => {
      const client = this.stripe;
      ...
      const subscription = await this.retrieveSubscriptionWithScheduleAndDiscounts(client, subscriptionId);
      ...
      if (schedule && ["active", "not_started"].includes(schedule.status)) {
        ...
        const fullSchedule = await client.subscriptionSchedules.retrieve(schedule.id, {
        ...
        if (nextPhase) {
          ...
          if (nonMeteredItem) {
            ...
            if (price?.product) {
              newProductId = typeof price.product === "string" ? price.product : price.product.id;
            }
          }
      ...
      const discounts = subscription.discounts
        .map((discount) => {
          ...
          const kind: "percent" | "amount" = percentOff !== null ? "percent" : "amount";
      ...
      try {
        ...
        const paymentMethods = await client.customers.listPaymentMethods(customerId, { limit: 1 });

        hasValidPaymentMethod = paymentMethods.data.length > 0;
      } catch (error) {
        logger.error("StripeBillingService.getSubscriptionInfo:failed to check payment method", {
          userId: this.ctx.session.user?.id,
          userEmail: this.ctx.session.user?.email,
  ...
  async getCustomerPortalUrl(orgId: string) {
    return await instrumentAsync({ name: "stripe.billingPortal.create", spanKind: SpanKind.CLIENT }, async (span) => {
      const client = this.stripe;

      const { parsedOrg } = await this.getParsedOrg(orgId);
      const stripeCustomerId = parsedOrg.cloudConfig?.stripe?.customerId;
      const stripeSubscriptionId = parsedOrg.cloudConfig?.stripe?.activeSubscriptionId;
  ...
  async createCheckoutSession(orgId: string, stripeProductId: string) {
    return await instrumentAsync({ name: "stripe.checkout.create", spanKind: SpanKind.CLIENT }, async (span) => {
      const client = this.stripe;
      ...
      if (parsedOrg.cloudConfig?.plan) {
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Cannot initialize stripe checkout for orgs that have a manual plan override",
        });
      }
      ...
      const product = await this.retrieveProductWithDefaultPrice(client, stripeProductId);
      ...
      const lineItems = await (async () => {
        ...
        const usageProduct = await this.retrieveProductWithDefaultPrice(client, StripeCatalogue.usageProductId());

        return [{ price: product.default_price.id, quantity: 1 }, { price: usageProduct.default_price.id }];
      })();

      const returnUrl = `${env.NEXTAUTH_URL}/organization/${orgId}/settings/billing`;
      const stripeCustomerId = parsedOrg.cloudConfig?.stripe?.customerId ?? undefined;
  ...
  async changePlan(orgId: string, newProductId: string, opId?: string) {
    return await instrumentAsync(
      ...
      async (span) => {
        ...
        const subscription = await this.retrieveSubscriptionWithScheduleAndDiscounts(client, stripeSubscriptionId);

        if (["canceled", "paused", "incomplete", "incomplete_expired"].includes(subscription.status)) {
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
        ...
        const newProduct = await this.retrieveProductWithDefaultPrice(client, newProductId);

        // If the users changes the plan, we want to cancel any pending cancellations
        const cancellationPayload = subscription.cancel_at ? { cancel_at: null } : { cancel_at_period_end: false };

        // [A] Legacy Plan Setup: Switch from / to legacy plans
        // ------------------------------------------------------------------------------------------------
        // TODO: Cleanup after all customers are migrated to the new system
        const isNewProductLegacy = newProduct.default_price.recurring?.usage_type === "metered";
        const isExistingSubscriptionLegacy = parsedOrg.cloudConfig?.stripe?.isLegacySubscription === true;
        ...
        if (isExistingSubscriptionLegacy || isNewProductLegacy) {
          ...
          const newLineItems = isNewProductLegacy
            ...
            : [{ price: newProduct.default_price.id, quantity: 1 }, { price: usageProduct.default_price.id }];
        ...
        const upgrading = StripeCatalogue.isUpgrade(currentSubscriptionProductId, newProductId);
  ...
  async cancel(orgId: string, opId?: string) {
    return await instrumentAsync({ name: "stripe.subscription.cancel", spanKind: SpanKind.CLIENT }, async (span) => {
      const client = this.stripe;
      ...
      try {
        ...
        logger.error("stripeBillingService.subscription.cancelAtPeriodEnd:failed", {
          subscriptionId,
          orgId: parsedOrg.id,
          userId: this.ctx.session.user.id,
          userEmail: this.ctx.session.user.email,
  ...
  async cancelImmediatelyAndInvoice(orgId: string, opId?: string) {
    return await instrumentAsync(
      ...
      async (span) => {
        ...
        const subscriptionId = parsedOrg.cloudConfig?.stripe?.activeSubscriptionId;
        if (!subscriptionId) {
          logger.info("stripeBillingService.subscription.cancel.now:noop.noActiveSubscription", {
            orgId,
          });
  ...
  async getInvoices(
    ...
    return await instrumentAsync({ name: "stripe.invoices.list", spanKind: SpanKind.CLIENT }, async (span) => {
      ...
      const preview = stripeSubscriptionId
        ? await this.createInvoicePreview(client, stripeCustomerId, stripeSubscriptionId)
        : null;
      ...
      const mapInvoiceToTableRow = async (invoice: Stripe.Invoice): Promise<InvoiceTableRow> => {
        ...
        const discountCents = Array.isArray((invoice as any).total_discount_amounts)
          ? (invoice as any).total_discount_amounts.reduce((acc: number, d: any) => acc + (d.amount ?? 0), 0)
          : 0;

        const taxCents = Array.isArray(invoice.total_taxes)
          ? invoice.total_taxes.reduce((acc: number, t: any) => acc + (t.amount ?? 0), 0)
          : 0;
        const totalCents = (typeof invoice.total === "number" ? invoice.total : invoice.amount_due) ?? 0;
      ...
      const invoices = await Promise.all(list.data.map((inv) => mapInvoiceToTableRow(inv)));
      ...
      if (showPreviewRow) {
        ...
        return {
          ...
          cursors: {
            next: modifiedInvoiceRows.length > 1 ? modifiedInvoiceRows[modifiedInvoiceRows.length - 1]!.id : undefined, // last real invoice id
      ...
      return {
        ...
        cursors: {
          next: list.has_more && invoices.length ? invoices[invoices.length - 1]!.id : undefined,
  ...
  async getUsage(orgId: string) {
    return await instrumentAsync({ name: "stripe.billing.getUsage", spanKind: SpanKind.CLIENT }, async (span) => {
      const client = this.stripe;
      ...
      if (stripeCustomerId && stripeSubscriptionId) {
        ...
        try {
          ...
          const usageItem = subscription.items.data.find((item) => this.isMetered(item.price));

          if (!usageItem) {
            throw new TRPCError({
              code: "INTERNAL_SERVER_ERROR",
              message: "Metered Cloud Usage Product item not found in subscription",
            });
          }

          // 1. Get the total usage off the preview Invoice
          const previewInvoice = await this.createInvoicePreview(client, stripeCustomerId, stripeSubscriptionId);
          ...
          const usageInvoiceLines = previewInvoice.lines.data.filter((line: any) => {
            const isMeteredLineItem = line.pricing?.price_details?.price === usageItem.price.id;
          ...
          logger.error("Failed to get usage from Stripe, using usage from Clickhouse", { error: e });
          traceException(e);
        }
      }
      // [A] End ----------------------------------------------------------------------------------------
    ...
    return await instrumentAsync(
      ...
      async (span) => {
        ...
        const existingDiscounts = (subscription.discounts || [])
          .map((discount) => {
            ...
            if (isExpandedOrNullable(promotionCode) && promotionCode) {
              const promoId = typeof promotionCode === "string" ? promotionCode : promotionCode.id;
              return {
          ...
          .filter((d): d is Stripe.SubscriptionUpdateParams.Discount => d !== undefined);
        ...
        try {
          ...
          logger.error("StripeBillingService.applyPromotionCode:stripe.subscription.update.failed", {
            userId: this.ctx.session.user?.id,
            userEmail: this.ctx.session.user?.email,
            customerId: subscription.customer,
            subscriptionId,
