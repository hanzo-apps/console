...
export const blobStorageIntegrationRouter = createTRPCRouter({
  ...
  update: protectedProjectProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const canUseHostCredentials = isSelfHosted && type === BlobStorageIntegrationType.S3;
        const isUsingHostCredentials = canUseHostCredentials && (!accessKeyId || !secretAccessKey);
        ...
        return await ctx.prisma.$transaction(async (prisma) => {
          ...
          if (existingConfig) {
            ...
            if (!isUsingHostCredentials && !secretAccessKey) {
              throw new TRPCError({
                code: "BAD_REQUEST",
                message: "Secret access key is required for new configuration when not using host credentials",
              });
            }
            ...
            return await prisma.blobStorageIntegration.create({
              data: {
                ...
                projectId: input.projectId,
                accessKeyId,
                secretAccessKey: secretAccessKey ? encrypt(secretAccessKey) : undefined,
              },
            });
          }
        });
  ...
  runNow: protectedProjectProcedure.input(z.object({ projectId: z.string() })).mutation(async ({ input, ctx }) => {
    try {
      throwIfNoProjectAccess({
        session: ctx.session,
        projectId: input.projectId,
      ...
      logger.info(`Manual blob storage integration job queued for project ${input.projectId}`);
  ...
  validate: protectedProjectProcedure.input(z.object({ projectId: z.string() })).mutation(async ({ input, ctx }) => {
    try {
      throwIfNoProjectAccess({
        session: ctx.session,
        projectId: input.projectId,
      ...
      const secretAccessKey = encryptedSecretAccessKey ? decrypt(encryptedSecretAccessKey) : undefined;
...
This file can be safely deleted.`;
      ...
      logger.info(`Blob storage validation successful for project ${input.projectId}`);
      ...
      logger.error(`Blob storage validation failed for project ${input.projectId}`, e);
