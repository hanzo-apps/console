...
export const SuccessNotification: React.FC<SuccessNotificationProps> = ({ title, description, onDismiss, link }) => {
  return (
    <div className="flex justify-between">
      <div className="flex min-w-[300px] flex-1 flex-col gap-2">
        <div className="flex items-center gap-2">
          <BadgeCheck size={20} className="text-primary-foreground" />
          <div className="m-0 text-sm font-medium leading-tight text-primary-foreground">{title}</div>
        </div>
        {description && <div className="text-sm leading-tight text-primary-foreground">{description}</div>}
        {link && (
