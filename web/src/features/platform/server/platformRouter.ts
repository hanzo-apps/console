...
export const platformRouter = createTRPCRouter({
  listContainers: protectedProjectProcedure
    .input(z.object({ projectId: z.string() }))
    .query(async () => {
      // Degrade to an empty list when PaaS is not configured / unreachable.
      let raw: unknown;
      try {
        raw = await paasGet<unknown>(buildContainerPath());
      } catch {
        return [] as PaasContainer[];
      }
  ...
  getContainer: protectedProjectProcedure
    ...
    .query(async ({ input }) => {
      const raw = await paasGet<Record<string, unknown>>(
        buildContainerPath(input.containerId),
      );
      return {
  ...
  listPipelines: protectedProjectProcedure
    ...
    .query(async ({ input }) => {
      let raw: unknown;
      try {
        raw = await paasGet<unknown>(
          buildContainerPath(`${input.containerId}/pipelines`),
        );
      } catch {
          ...
          durationMs:
            typeof r.durationMs === "number"
              ...
              : typeof r.duration_ms === "number"
                ...
                : undefined,
          logsUrl:
            typeof r.logsUrl === "string"
              ? r.logsUrl
              : typeof r.logs_url === "string"
                ? r.logs_url
                : undefined,
        }),
      );
  ...
  triggerBuild: protectedProjectProcedure
    ...
    .mutation(async ({ input }) => {
      return paasPost<{ ok: boolean }>(
        buildContainerPath(`${input.containerId}/redeploy`),
      );
    }),
});
...
function mapContainerStatus(raw: unknown): PaasContainer["status"] {
  ...
  if (s.includes("deploy") || s.includes("build") || s.includes("pend"))
    return "deploying";
