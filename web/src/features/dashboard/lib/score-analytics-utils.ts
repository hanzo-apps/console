...
import { type CategoryCounts, type ChartBin, type HistogramBin } from "@/src/features/scores/types";
...
function computeBinSize(minBins: number, maxBins: number, range: number, valueCount: number) {
  return range === 0 ? 1 : Math.min(Math.max(minBins, Math.floor(Math.sqrt(valueCount))), maxBins);
}

export function createHistogramData(data: DatabaseRow[], minBins = 1, maxBins = 10) {
  const numericScoreValues = data.map((item) => item.value as number);
  if (!Boolean(numericScoreValues.length)) return { chartData: [], chartLabels: [] };

  const min = round(Math.min(...numericScoreValues));
  const range = round(Math.max(...numericScoreValues)) - min;
  const bins = computeBinSize(minBins, maxBins, range, numericScoreValues.length);
  const binSize = range / bins || 1;

  const baseChartData = Array.from({ length: bins }).map((_, index: number) => ({
    count: 0,
...
function groupCategoricalScoreDataByTimestamp(
  ...
  return data.reduce(
    (acc, row) => {
      ...
      const timestamp = new Date((row[scoreTimestampAccessor] as string) ?? new Date());
...
type ChartAccumulator = Map<string, { chartData: ChartBin[]; chartLabels: string[] }>;
...
function addMetricToAccumulator(acc: ChartAccumulator, key: string, chartBin: ChartBin, chartLabels: string[]) {
...
export function transformAggregatedRunMetricsToChartData(
  ...
  return reversedMetrics.reduce((acc, run) => {
    ...
    Object.entries(run.scores ?? {}).forEach(([scoreId, score]) => {
      const scoreData =
        score.type === "NUMERIC"
          ? createNumericScoreData(run.name, score.average, scoreIdToName.get(scoreId) ?? scoreId)
          : createCategoricalScoreData(run.name, score.valueCounts, score.values);

      addMetricToAccumulator(acc, scoreId, scoreData.chartBin, scoreData.chartLabels);
    });

    // Handle resource metrics
    RESOURCE_METRICS.forEach(({ key, objectKey }) => {
      const resourceValue = run[objectKey as keyof typeof run];
      const resourceData = createNumericScoreData(run.name, !!resourceValue ? Number(resourceValue) : 0, key);

      addMetricToAccumulator(acc, key, resourceData.chartBin, resourceData.chartLabels);
    });
...
export function transformCategoricalScoresToChartData(
  ...
  if (!agg) {
    ...
    const scoreDataByTimestamp = groupCategoricalScoreDataByTimestamp(data, scoreTimestampAccessor);
...
export function isEmptyChart({ data }: { data: ChartBin[] }) {
  return data.length === 0 || data.every((item) => Object.keys(item).length === 1);
}
