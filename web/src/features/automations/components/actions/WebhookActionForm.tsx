  ...
  headers: z.array(
    z.object({
      name: z.string().refine(
        ...
        {
          message: "This header is automatically added by Hanzo and cannot be customized",
        },
      ),
      value: z.string(),
...
export const WebhookActionForm: React.FC<WebhookActionFormProps> = ({ form, disabled, projectId, action }) => {
  const {
    fields: headerFields,
    append: appendHeader,
    remove: removeHeader,
  ...
  return (
    <div className="space-y-4">
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            ...
            <FormDescription>
              The HTTP URL to call when the trigger fires. We will send a POST request to this URL. Only HTTPS URLs are
      ...
      <div>
        ...
        <div className="mb-4">
          ...
          {Object.entries({
            ...
            <div key={key} className="mb-2 grid grid-cols-[1fr,1fr,auto,auto] gap-2">
              ...
              <FormItem>
                <FormControl>
                  <Input value={value} disabled={true} className="bg-muted/50" />
                </FormControl>
              </FormItem>
            </div>
          ))}
        ...
        <FormDescription className="mb-2">Optional custom headers to include in the webhook request:</FormDescription>
        ...
        {customHeaderFields.map((field) => {
          ...
          const isSecret = form.watch(`webhook.headers.${originalIndex}.isSecret`);
          const displayValue = form.watch(`webhook.headers.${originalIndex}.displayValue`);

          return (
            <div key={field.id} className="mb-2 grid grid-cols-[1fr,1fr,auto,auto] gap-2">
              <FormField
                control={form.control}
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
                      <Input placeholder="Header Name" {...field} disabled={disabled} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
                      <Input
                        placeholder={isSecret && displayValue ? displayValue : displayValue || "Value"}
                        {...field}
                        disabled={disabled}
        ...
        <Button type="button" variant="outline" onClick={addHeader} disabled={disabled} className="mt-2">
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1">
                <CodeView
                  ...
                  content={(action.config as SafeWebhookActionConfig).displaySecretKey}
                  defaultCollapsed={false}
                />
              </div>
              <div className="flex gap-2">
                <RegenerateWebhookSecretButton projectId={projectId} action={action} />
              </div>
            </div>
...
export const RegenerateWebhookSecretButton = ({
  ...
  const regenerateSecretMutation = api.automations.regenerateWebhookSecret.useMutation({
    onSuccess: (data) => {
      showSuccessToast({
    ...
    <>
      <Popover open={showConfirmPopover} onOpenChange={setShowConfirmPopover}>
        <PopoverTrigger asChild>
          <Button type="button" variant="outline" size="default" disabled={regenerateSecretMutation.isPending}>
            <RefreshCw className={`mr-2 h-4 w-4 ${regenerateSecretMutation.isPending ? "animate-spin" : ""}`} />
            Regenerate
          </Button>
        </PopoverTrigger>
        <PopoverContent>
          ...
          <p className="mb-3 max-w-sm text-sm">
            This action will invalidate the current webhook secret and generate a new one. Any existing integrations
      ...
      <Dialog open={showRegenerateDialog} onOpenChange={setShowRegenerateDialog}>
        <DialogContent className="max-w-4xl">
          ...
          <DialogBody>{regeneratedSecret && <WebhookSecretRender webhookSecret={regeneratedSecret} />}</DialogBody>
          <DialogFooter>
            <Button
              onClick={() => {
