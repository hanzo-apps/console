...
function KeyValueEntryRow({
  ...
  return (
    <div
      className={`bg-background space-y-2 rounded-md border p-3 ${isSchemaField ? "border-primary/30" : ""}`}
    >
      <div className="grid grid-cols-[1fr,auto] gap-2">
        ...
        <div className="flex items-end">
          ...
          >
            <Trash2
              className={`h-4 w-4 ${!canRemove ? "text-muted-foreground/30" : "text-muted-foreground"}`}
            />
          </Button>
        </div>
      </div>
