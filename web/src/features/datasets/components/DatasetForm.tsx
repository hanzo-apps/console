...
type DatasetFormProps = CreateDatasetFormProps | UpdateDatasetFormProps | DeleteDatasetFormProps;
...
export const DatasetForm = forwardRef<DatasetFormRef, DatasetFormProps>((props, ref) => {
  ...
  const inputSchemaString =
    props.mode === "update" && props.datasetInputSchema ? JSON.stringify(props.datasetInputSchema, null, 2) : "";
    ...
    defaultValues:
      props.mode === "update"
        ? {
            ...
            metadata: props.datasetMetadata ? JSON.stringify(props.datasetMetadata, null, 2) : "",
            inputSchema: inputSchemaString,
            expectedOutputSchema: expectedOutputSchemaString,
          }
        : {
            name: props.mode === "create" && props.folderPrefix ? `${props.folderPrefix}/` : "",
            description: "",
            metadata: "",
            inputSchema: "",
  ...
  useEffect(() => {
    if (onValidationChange) {
      ...
      if (!prev || prev.isValid !== isValid || prev.isSubmitting !== isSubmitting) {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const inputSchema = values.inputSchema === "" ? null : JSON.parse(values.inputSchema);
    const expectedOutputSchema = values.expectedOutputSchema === "" ? null : JSON.parse(values.expectedOutputSchema);
        ...
        .then((result) => {
          if (result.success) {
            ...
            if (props.redirectOnSuccess !== false) {
              router.push(`/project/${props.projectId}/datasets/${result.dataset.id}/items`);
            }
          } else {
  ...
  return (
    <Form {...form}>
      <form
        onSubmit={props.mode === "delete" ? handleDelete : form.handleSubmit(onSubmit)}
        className="flex h-full min-h-0 flex-col"
      >
        <DialogBody className={props.showFooter === false ? "p-0" : undefined}>
          {props.mode === "delete" ? (
            <div className="mb-8 grid w-full gap-1.5">
              <Label htmlFor="delete-confirmation">Type &quot;{props.datasetName}&quot; to confirm deletion</Label>
              <Input
            ...
            <div className="mb-8 space-y-6">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Use slashes &apos;/&apos; in dataset names to organize them into <em>folders</em>.
                    </FormDescription>
                    <FormControl>
                      <Input {...field} />
                    </FormControl>
              ...
              {serverSideSchemaValidationErrors && (
                <DatasetSchemaValidationError
                  projectId={props.projectId}
                  datasetId={props.mode === "update" ? props.datasetId : "unknown"}
