...
import { CreateLlmApiKey, UpdateLlmApiKey } from "@/src/features/llm-api-key/types";
...
export function getDisplaySecretKey(secretKey: string) {
  ...
  return secretKey.endsWith('"}') ? "..." + secretKey.slice(-6, -2) : "..." + secretKey.slice(-4);
}
...
async function testLLMConnection(params: TestLLMConnectionParams): Promise<{ success: boolean; error?: string }> {
  try {
    const model = params.customModels?.length ? params.customModels[0] : supportedModels[params.adapter][0];
    ...
    if (params.adapter === LLMAdapter.VertexAI) {
      ...
      if (params.secretKey !== VERTEXAI_USE_DEFAULT_CREDENTIALS) {
        const parsed = GCPServiceAccountKeySchema.safeParse(JSON.parse(params.secretKey));
    ...
    if (params.config && params.adapter === LLMAdapter.Bedrock) {
      ...
      parsedConfig = vertexAIConfig.location ? { location: vertexAIConfig.location } : null;
    }
...
export const llmApiKeyRouter = createTRPCRouter({
  create: protectedProjectProcedureWithoutTracing.input(CreateLlmApiKey).mutation(async ({ input, ctx }) => {
    try {
      ...
      const key = await ctx.prisma.llmApiKeys.create({
        data: {
          ...
          extraHeaders: input.extraHeaders ? encrypt(JSON.stringify(input.extraHeaders)) : undefined,
          extraHeaderKeys: input.extraHeaders ? Object.keys(input.extraHeaders) : undefined,
  ...
  test: protectedProjectProcedureWithoutTracing.input(CreateLlmApiKey).mutation(async ({ input }) => {
  ...
  testUpdate: protectedProjectProcedureWithoutTracing.input(UpdateLlmApiKey).mutation(async ({ input, ctx }) => {
    try {
      throwIfNoProjectAccess({
        session: ctx.session,
        projectId: input.projectId,
      ...
      const decryptedSecretKey =
        input.secretKey !== undefined && input.secretKey !== "" && input.secretKey !== null
          ? input.secretKey
      ...
      const extraHeaders =
        input.extraHeaders ??
        (existingKey.extraHeaders ? decryptAndParseExtraHeaders(existingKey.extraHeaders) : undefined);
  ...
  update: protectedProjectProcedureWithoutTracing.input(UpdateLlmApiKey).mutation(async ({ input, ctx }) => {
    try {
      throwIfNoProjectAccess({
        session: ctx.session,
        projectId: input.projectId,
      ...
      const decryptedHeaders = existingKey.extraHeaders ? decryptAndParseExtraHeaders(existingKey.extraHeaders) : null;
      ...
      if (input.extraHeaders === undefined) {
        ...
        extraHeaders = Object.keys(existingHeaders).length > 0 ? existingHeaders : undefined;
      } else {
      ...
      const key = await ctx.prisma.llmApiKeys.update({
        ...
        data: {
          ...
          extraHeaders: extraHeaders ? encrypt(JSON.stringify(extraHeaders)) : undefined,
          extraHeaderKeys: extraHeaders ? Object.keys(extraHeaders) : undefined,
          displaySecretKey: input.secretKey ? getDisplaySecretKey(input.secretKey) : undefined,
          baseURL: input.baseURL,
          withDefaultModels: input.withDefaultModels,
          customModels: input.customModels,
          config: input.config,
