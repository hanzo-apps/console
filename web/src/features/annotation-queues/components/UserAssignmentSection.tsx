...
export const UserAssignmentSection = ({
  ...
  const queueAssignmentsQuery = api.annotationQueueAssignments.byQueueId.useQuery(
    { projectId, queueId: queueId as string },
    { enabled: !!queueId && hasQueueAssignmentsReadAccess },
  );

  const deleteQueueAssignmentMutation = api.annotationQueueAssignments.delete.useMutation({
    onSuccess: () => {
  ...
  const assignedUserIds = queueAssignmentsQuery.data?.assignments.map((user: any) => user.id) || [];
  ...
  const hasMoreAssignedUsers =
    queueAssignmentsQuery.data && queueAssignmentsQuery.data.totalCount > queueAssignmentsQuery.data.assignments.length;
          ...
          >
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-3">
                <p className="truncate text-xs font-medium">{user.name || "Unnamed User"}</p>
                <p className="truncate text-xs text-muted-foreground">{user.email}</p>
              </div>
            </div>
            {isSelected && <div className="text-xs text-muted-foreground">✓</div>}
          </div>
        )}
      />

      {/* Assigned Users Section */}
      {queueAssignmentsQuery.data && queueAssignmentsQuery.data?.totalCount > 0 && (
        <div className="space-y-2">
          <h4 className="text-sm text-muted-foreground">Assigned to ({queueAssignmentsQuery.data?.totalCount})</h4>
              ...
              <div key={user.id}>
                <div className="flex items-center justify-between gap-3 px-3 py-2">
                  ...
                  <Button
                    variant="ghost"
                    size="icon-sm"
                    disabled={!hasQueueAssignmentWriteAccess || deleteQueueAssignmentMutation.isPending}
              ...
              <div className="flex items-center gap-3 px-3 py-2 text-muted-foreground">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-xs italic">
                    {queueAssignmentsQuery.data.totalCount - queueAssignmentsQuery.data.assignments.length} more
                    assigned users
                  </p>
                </div>
              </div>
