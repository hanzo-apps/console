...
export const vectorRouter = createTRPCRouter({
  ...
  stats: protectedProjectProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        return await vectorGet<VectorStats>("/api/vector/stats", apiKey, {
          projectId: input.projectId,
        });
      } catch {
        // Degrade to an honest empty state when Vector is not configured.
        return {
          totalCollections: 0,
          totalVectors: 0,
          totalStorageBytes: 0,
  ...
  listCollections: protectedProjectProcedure
    ...
    .query(async ({ input }) => {
      try {
        const apiKey = resolveApiKey();
        return await vectorGet<{ collections: VectorCollection[] }>(
  ...
  createCollection: protectedProjectProcedure
    ...
    .mutation(async ({ input }) => {
      const apiKey = resolveApiKey();
      return vectorPost<{ collection: VectorCollection }>(
        "/api/vector/collections",
        apiKey,
        {
          name: input.name,
  ...
  deleteCollection: protectedProjectProcedure
    ...
    .mutation(async ({ input }) => {
      const apiKey = resolveApiKey();
      return vectorDelete<{ success: boolean }>(
        `/api/vector/collections/${encodeURIComponent(input.name)}`,
        apiKey,
        {
          projectId: input.projectId,
        },
