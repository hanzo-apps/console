...
export const ExecutionQueue = forwardRef<
  ExecutionQueueRef,
  ExecutionQueueProps
>(
  (
    { reasonerId, maxConcurrent = 5, onExecutionComplete, onExecutionSelect },
    ref,
  ) => {
    ...
    const executeReasoner = async (executionId: string, input: any) => {
      try {
        ...
        const asyncResponse = await reasonersApi.executeReasonerAsync(
          reasonerId,
          request,
        );
        ...
        const pollForCompletion = async () => {
          ...
          while (attempts < maxAttempts) {
            try {
              const statusResponse = await reasonersApi.getExecutionStatus(
                asyncResponse.execution_id,
              );
              ...
              if (statusResponse.status === "completed") {
                ...
                if (completedExecution && onExecutionComplete) {
                  onExecutionComplete({
                    ...
                    workflow_id:
                      asyncResponse.workflow_id ??
                      completedExecution.workflow_id,
                    result: statusResponse.result,
                    endTime,
                    duration,
                  });
    ...
    const checkPageAvailability = async (
      ...
      try {
        ...
        if (retryCount < maxRetries) {
          ...
          console.warn(
            `Page availability check failed for execution ${backendExecutionId} after ${maxRetries} retries`,
          );
        }
      }
    };
            ...
            <CardContent className="space-y-2">
              {activeExecutions.map((execution) => (
                <div
                  ...
                  onClick={
                    execution.execution_id
                      ? (e) => handleExecutionCardClick(execution, e)
                      : undefined
                  }
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 text-sm font-medium">
                        ...
                        {execution.execution_id && (
                          <span className="text-primary/70 bg-primary/10 rounded-md px-1.5 py-0.5 text-xs">
                            navigable
                          </span>
                        )}
                      </div>
                      ...
                      <>
                        {execution.checking_availability ? (
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={(e) =>
                              handleViewDetailsClick(e, execution)
                            }
                ...
                <div
                  ...
                  className={`bg-card flex items-center justify-between rounded-lg border p-3 transition-colors ${
                    ...
                    selectedExecution === execution.id
                      ? "ring-primary/20 bg-accent/30 ring-2"
                      : ""
                  }`}
