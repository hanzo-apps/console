    ...
    switch (status) {
      ...
      case "inactive":
        return {
          ...
          className:
            "bg-status-warning-bg text-status-warning border-status-warning-border font-medium",
        };
      case "revoked":
        return {
          ...
          icon: ErrorFilled,
          className:
            "bg-status-error-bg text-status-error border-status-error-border font-medium",
        };
      default:
        return {
          ...
          icon: Unknown,
          className:
            "bg-status-neutral-bg text-status-neutral border-status-neutral-border font-medium",
        };
    }
  };
...
export function DIDCountBadge({
  ...
  const typeConfig = {
    reasoners: {
      ...
      icon: Cognitive,
      color: "bg-muted text-muted-foreground border-border font-medium",
    },
    skills: {
  ...
  return (
    <Badge
      variant="outline"
      className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs transition-colors duration-150 ${config.color} ${className}`}
    >
      <config.icon size={12} />
      {count} {config.label}
    </Badge>
...
export function DIDIdentityBadge({
  ...
  return (
    <div className={`group inline-flex items-center gap-2 ${className}`}>
      <Badge
        variant="outline"
        className="bg-muted text-foreground border-border hover:bg-card-hover inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 font-mono text-xs transition-colors duration-150"
        title={did}
      >
      ...
      {showCopyButton && (
        <button
          onClick={handleCopy}
          className="text-muted-foreground hover:text-foreground hover:bg-muted flex h-6 w-6 items-center justify-center rounded-md opacity-0 transition-colors duration-150 group-hover:opacity-100"
    ...
    return (
      <div className={`inline-flex items-center gap-1 ${className}`}>
        ...
        {(reasonerCount > 0 || skillCount > 0) && (
          <Badge
            variant="outline"
            className="bg-gray-50 text-xs dark:border-gray-700 dark:bg-gray-950/20 dark:text-gray-400"
          >
