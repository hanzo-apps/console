...
const CreateTextPromptBaseSchema = z.object({
  ...
  prompt: z.string().describe("The prompt text content (supports {{variables}})"),
...
export const [createTextPromptTool, handleCreateTextPrompt] = defineTool({
  ...
  handler: async (input, context) => {
    return await instrumentAsync({ name: "mcp.prompts.create_text", spanKind: SpanKind.INTERNAL }, async (span) => {
