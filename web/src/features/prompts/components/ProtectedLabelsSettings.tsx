...
export default function ProtectedLabelsSettings({ projectId }: { projectId: string }) {
  ...
  const { data: protectedLabels = [] } = api.prompts.getProtectedLabels.useQuery(
    { projectId },
    {
  ...
  const availableLabels = allLabels.filter(
    (label) => !protectedLabels.includes(label) && label !== LATEST_PROMPT_LABEL,
  );
    ...
    <div>
      ...
      <Card className="mb-4 p-3">
        <p className="mb-4 text-sm text-primary">
          Protected labels can only be modified by users with admin or owner access. This prevents other users from
          changing or removing these labels from prompts.
        </p>
            ...
            >
              {hasAccess && hasEntitlement && (
                <Button
                  ...
                  onClick={() => {
                    if (confirm(`Are you sure you want to remove the protected label "${label}"?`)) {
                      ...
                      <Command>
                        ...
                        <CommandGroup>
                          {availableLabels.map((label) => (
                            ...
                            >
                              <Check
                                className={cn("mr-2 h-4 w-4", field.value === label ? "opacity-100" : "opacity-0")}
                              />
                              {label}
                            </CommandItem>
                          ))}
